...
pub async fn execute(ctx: &super::ExecContext, cmd: ContactsCommand) -> anyhow::Result<()> {
    ...
    match cmd {
        ...
        ContactsCommand::Add { contact_ids, standing, label_ids, watched } => {
            let labels = if label_ids.is_empty() { None } else { Some(label_ids.as_slice()) };
            let watched = if watched { Some(true) } else { None };
            ctx.client.add_contacts(cid, standing, &contact_ids, labels, watched).await?;
            println!("Contacts added.");
            Ok(())
        }
        ContactsCommand::Edit { contact_ids, standing, label_ids, watched } => {
            let labels = if label_ids.is_empty() { None } else { Some(label_ids.as_slice()) };
            let watched = if watched { Some(true) } else { None };
            ctx.client.edit_contacts(cid, standing, &contact_ids, labels, watched).await?;
